...
int example_switch_default_vlan_config(int unit)
{
  ...
  if (rv != OPENNSL_E_NONE) {
	  dzlog_error("Failed to get port configuration. Error %s\n", opennsl_errmsg(rv));
    return rv;
  }

  rv = opennsl_vlan_port_add(unit, DEFAULT_VLAN, pcfg.e, pcfg.e);
  if (rv != OPENNSL_E_NONE) {
	  dzlog_error("Failed to add ports to VLAN. Error %s\n", opennsl_errmsg(rv));
    return rv;
  }
...
void l2_print_mac(char *str, opennsl_mac_t mac_address){
  ...
  dzlog_info("%s %02x:%02x:%02x:%02x:%02x:%02x",
      str,
      a,b,c,
      d,e,f);
  return;
...
int opennsl_ip_parse(char *ip_str, unsigned int *ip_val)
{
  ...
  char *tok;
  int count = 0;
  char buf[16]; /* Maximum length of IP address in dotted notation */
...
void print_ip_addr(char *str, unsigned int host)
{
  ...
  c = (host >> 8 ) & 0xff;
  d = host & 0xff;
  dzlog_info("%s %d.%d.%d.%d", str, a,b,c,d);
}
